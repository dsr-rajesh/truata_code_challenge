"""
    ...
    Task2: Aggregate operations on DF
"""
import time
import logging
import sys
...
def save_txt(dest_df):
    ...
    dest_df.coalesce(1).write.format("csv").option("header", "true").mode("append").save(
        "C:/Users/DSR/PycharmProjects/truata_code_challenge/tmp/task_2")
    os.rename(
        "C:/Users/DSR/PycharmProjects/truata_code_challenge/tmp/task_2/part-00000-5643c768-e709-4c0c-9fc3-bf99626f3142-c000.csv",
        "C:/Users/DSR/PycharmProjects/truata_code_challenge/out/out_2_2.csv")
...
if __name__ == '__main__':
    ...
    log.info(" TASK_2_2 Program Started")
    try:
        spark = spark_init("Truta: TASK_2_2")

        # Loading Source data
        src_df = read_data(spark, file_path)
        max_price = (src_df.agg({"price": "max"}).collect()[0])["max(price)"]
        min_price = (src_df.agg({"price": "min"}).collect()[0])["min(price)"]
        row_count = src_df.distinct().count()

        # New DF creation
        data = [Row(min_price=min_price, max_price=max_price, row_count=row_count)]
        dest_df = spark.createDataFrame(data)
        dest_df.show()
        #Save DF to file
        save_txt(dest_df)

        log.info(" TASK_2_2 : DF transform completed")
